 ...
 * @return bool
 */
function doublee_is_plugin_active($plugin_file) {
	static $plugins = null;
 ...
 * @return string
 */
function doublee_parse_mime($mime) {
 ...
 */
function doublee_get_excerpt_from_acf($excerpt) {

	if(!$excerpt) {
		// Find the first set of flexible modules, if there are any
		$field_name = doublee_get_name_of_first_acf_field_name_of_type('flexible_content');
		...
		if($field_name && $modules) {
			$excerpt = doublee_get_first_acf_subfield_value_of_type($modules, array('wysiwyg', 'textarea'), $field_name);
		}
	}

	return $excerpt;
}
add_filter('get_the_excerpt', 'doublee_get_excerpt_from_acf', 10, 1);
 ...
 */
function doublee_get_primary_term($taxonomy, $id = false) {
	$id = ($id ? $id : get_the_ID());
 ...
 * @return mixed|string
 */
function doublee_reroute_to_404($template) {
 ...
 */
function doublee_redirect_to_url() {
