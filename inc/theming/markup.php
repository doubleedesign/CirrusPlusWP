 ...
 */
function doublee_post_classes($classes) {
	return $classes;
}
add_filter('post_class', 'doublee_post_classes');
 ...
 */
function doublee_filter_ptags_on_images($content){
	return preg_replace('/<p>\s*(<a .*>)?\s*(<img .* \/>)\s*(<\/a>)?\s*<\/p>/iU', '\1\2\3', $content);
}
add_filter('the_content', 'doublee_filter_ptags_on_images');
 ...
 * @return string|string[]|null
 */
function doublee_filter_div_on_table($content){
	return preg_replace('/(<table.*<\/table\>)/si','<div class="table-wrap">$0</div>', $content);
}
add_filter('the_content', 'doublee_filter_div_on_table');
 ...
 * @return string
 */
function doublee_filter_div_on_oembed($html, $url, $attr, $post_id) {
	return '<div class="oembed">'.$html.'</div>';
}
add_filter('embed_oembed_html', 'doublee_filter_div_on_oembed', 100, 4);